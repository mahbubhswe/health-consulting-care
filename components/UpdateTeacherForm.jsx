...
export default function CreateStudentForm({ data }) {
  ...
  const {
    ...
    defaultValues: {
      id: data.id,
      name: data.name,
      teacherInitial: data.teacherInitial,
      phone: data.phone,
      email: data.email,
      gender: data.gender,
  ...
  const onSubmit = async (data) => {
    Swal.fire({
      title: "Are you sure",
      text: "Do you want to update teacher information",
      icon: "question",
      ...
      if (result.isConfirmed) {
        try {
          setOpen(true);
          data.avatar = avatar;
          const apiRes = await axios.put(`/api/teacher/update`, data, {
            headers: {
          ...
          setOpen(false);
          if (
            apiRes.data == "Teacher's information has been updated successfully"
          ) {
